...
class StatusStream extends Readable {
  ...
  extractStartedTasks(message) {

    const taskRegex = /has\sstarted\s[\d*]\stasks\:\s(?:\(task\s([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})\)\s?)+/
    return message.match(taskRegex);
  }
