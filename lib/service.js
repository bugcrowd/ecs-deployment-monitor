...
class Service extends EventEmitter {
  ...
  _targets(cb) {
    ...
    const lbs = _.filter(this.raw.loadBalancers, (lb) => !!lb.targetGroupArn);
  ...
  _emitNewEvents() {
    ...
    let events = _.filter(this.raw.events, (event) => event.createdAt > this.eventsAfter);
  ...
  _convertEvent(rawEvent, cb) {
    ...
    const eventClass = _.find(eventClassTestOrder, (type) => type.test(rawEvent));
