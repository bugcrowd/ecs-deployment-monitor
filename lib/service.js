...
class Service extends EventEmitter {
  constructor(options) {
    ...
    options.durationBetweenPolls = options.durationBetweenPolls || 3000;
  ...
  _emitNewEvents() {
    ...
    const events = this.raw.events
      .filter((event) => event.createdAt > this.eventsAfter)
      .sort((a, b) => { return a.createdAt - b.createdAt });

    if (events.length > 0) this.eventsAfter = events[events.length - 1].createdAt;
