...
class Deployment extends EventEmitter {
  constructor(options) {
    ...
    // defaults
    options.durationBetweenPolls = options.durationBetweenPolls || 1000;
    options.failureThreshold = options.failureThreshold || 0.25;
    ...
    this.on('updated', () => {
      this.evaluate(evaluators, () => { });
    });
  }

  _serviceUpdated() {
    this.raw = this.service.raw.deployments.find((d) => {
    ...
    this.stoppedTasks((err, tasks) => {
      ...
      var taskArns = tasks.map(task => task.taskArn);
      logger.log({ level: 'debug', type: 'deployment', message: `Tasks Stopped: ${taskArns.join(', ')}` });

      this.tasksFailed = tasks.map(task => task.taskArn);
  ...
  _serviceEventListener(event) {
    ...
    var filterDeploymentTasks = (tasks) => {
      return event.tasks.filter(task => this.doesTaskBelong(task));
    }

    if (event instanceof events.TasksStartedEvent) {
      let deploymentTasks = filterDeploymentTasks(event.tasks);
      let deploymentTasksArns = deploymentTasks.map(task => task.taskArn);
  ...
  evaluate(evaluators, cb) {
    ...
    async.eachSeries(Deployment.evaluationOrder, (state, done) => {
      const evaluator = evaluators[state];
  ...
  // Produces an array that is the set difference between two arrays.
  static _difference(arr1, arr2) {
    const diff1 = arr1.filter(value => !arr2.includes(value));
   ...
   */
  activeTasks() {
    return Deployment._difference(this.tasksStarted, this.tasksFailed);
  }
  ...
  stoppedTasks(cb) {
    ...
    taskLoader(this.service, this.tasksStarted, (err, tasks) => {
      ...
      cb(null, tasks.filter(task => task.lastStatus === 'STOPPED'));
    });
  }
   ...
   */
  hasState(state) {
    return !!this.history.find(item => item.state === state);
  }
}
